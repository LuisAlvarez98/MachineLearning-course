...
class Knn:
    ...
    def euclidean_distance(self, example1, example2):
        ...
        return np.sqrt(np.sum((example2 - example1)**2))
    ...
    def get_neighbors(self, example):
        ...
        distances = [ self.euclidean_distance(i, example) for i in self.model_x ]
        
        neighbor_indices    = np.argsort(distances)[:self.k]
        neighbor_labels     = [ self.model_y[i][0] for i in neighbor_indices ]
